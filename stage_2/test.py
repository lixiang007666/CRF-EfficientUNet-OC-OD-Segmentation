...
from nnunet.utilities.file_conversions import convert_2d_image_to_nifti,convert_2d_image_to_nifti_lx
...
if __name__ == '__main__':
    ...
    base = '/home/haishan/Data/dataLiXiang/nnunetData'
    # this folder should have the training and testing subfolders
    nnUNet_raw_data = "/home/haishan/Data/dataLiXiang/DATASET/nnUNet_raw/nnUNet_raw_data"
    # now start the conversion to nnU-Net:
    task_name = 'Task008_Refuge'
    ...
    training_cases = subfiles(images_dir_tr, suffix='.jpg', join=False)
    training_cases2 = subfiles(labels_dir_tr, suffix='.bmp', join=False)
    ...
    for t in training_cases2:
        ...
        # the labels are stored as 0: background, 255: road. We need to convert the 255 to 1 because nnU-Net expects
        # the labels to be consecutive integers. This can be achieved with setting a transform
        convert_2d_image_to_nifti_lx(input_segmentation_file, output_seg_file, is_seg=True)

    # now do the same for the test set
    labels_dir_ts = join(base, 'testing', 'output')
    images_dir_ts = join(base, 'testing', 'input')
    testing_cases = subfiles(images_dir_ts, suffix='.jpg', join=False)
    testing_cases2 = subfiles(labels_dir_ts, suffix='.bmp', join=False)
    ...
    once that is completed, you can run the trainings s follows:
